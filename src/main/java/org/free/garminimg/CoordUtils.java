...
{
    public static boolean matchesCoordinates(int minLong1, int maxLong1, int minLat1, int maxLat1, int minLong2, int maxLong2, int minLat2, int maxLat2)
    {
        assert (minLong1<=maxLong1);
        assert (minLat1<=maxLat1);
        assert (minLong2<=maxLong2);
        assert (minLat2<=maxLat2);
        return !(minLong1>maxLong2 || maxLong1<minLong2 || minLat1>maxLat2 || maxLat1<minLat2);
    }

    /**
     * @return true if 1 is included in 2
     */
    public static boolean includedInCoordinates(int minLong1, int maxLong1, int minLat1, int maxLat1, int minLong2, int maxLong2, int minLat2, int maxLat2)
    {
        assert (minLong1<=maxLong1);
        assert (minLat1<=maxLat1);
        assert (minLong2<=maxLong2);
        assert (minLat2<=maxLat2);
        return minLong1>=minLong2 && maxLong1<=maxLong2 && minLat1>=minLat2 && maxLat1<=maxLat2;
    }
    ...
    public static String toDMS(int coord)
    {
        ...
        long min=tmp/(60*100);
        long sec=tmp%(60*100);
        return String.format("%s%dd%02d'%02.2f", (negative?"-":""), deg, min, sec/100.0);
    }

    public static boolean includedInCoordinates(int longitude, int latitude, int minLon, int maxLon, int minLat, int maxLat)
    {
        return longitude>=minLon && longitude<=maxLon && latitude>=minLat && latitude<=maxLat;
    }

    public static boolean includedInCoordinates(int[] longitudes, int[] latitudes, int minLon, int maxLon, int minLat, int maxLat)
    {
        ...
        return includedInCoordinates(minLon1, maxLon1, minLat1, maxLat1, minLon, maxLon, minLat, maxLat);
    }

    public static boolean matchesCoordinates(int[] longitudes, int[] latitudes, int nbPoints, int minLon, int maxLon, int minLat, int maxLat)
    {
        int minLon1=Integer.MAX_VALUE;
        int maxLon1=Integer.MIN_VALUE;
        int minLat1=Integer.MAX_VALUE;
        ...
        return matchesCoordinates(minLon1, maxLon1, minLat1, maxLat1, minLon, maxLon, minLat, maxLat);
    }
}
