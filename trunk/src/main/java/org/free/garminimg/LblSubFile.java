...
class LblSubFile extends ImgSubFile
{
    ...
    public LblSubFile(String filename, String filetype, int fileSize, int blocSize, ImgFileBag fileBag)
    {
    ...
    public void printDebug(PrintStream out) throws IOException
    {
        ...
        out.println("  dataOffset="+dataOffset);
        out.println("  dataLength="+dataLength);
        out.println("  labelCoding="+labelCoding);

        out.println("  countryDefinitionOffset="+countryDefinitionOffset);
        out.println("  countryDefinitionLength="+countryDefinitionLength);
        out.println("  countryDefinitionSize="+countryDefinitionSize);

        out.println("  regionDefinitionOffset="+regionDefinitionOffset);
        out.println("  regionDefinitionLength="+regionDefinitionLength);
        out.println("  regionDefinitionSize="+regionDefinitionSize);

        out.println("  cityDefinitionOffset="+cityDefinitionOffset);
        out.println("  cityDefinitionLength="+cityDefinitionLength);
        out.println("  cityDefinitionSize="+cityDefinitionSize);
    }
    ...
    public void getPOI(long offset, POILabel poi) throws IOException
    {
        ...
        {
            poi.setCity(getCity(((cityDefinitionLength/cityDefinitionSize>255)?readUInt16(context):readByte(context)), poi, context));
        }
        if((propertyMask&HAS_ZIP)!=0)
        {
            poi.setZip(getZip(zipLength/zipSize>255?readUInt16(context):readByte(context), context));
        }
    ...
    private String getCity(int index, POILabel poi, FileContext context) throws IOException
    {
        ...
        else
        {
            ...
            Label label=poi.getRgn().getIndexPointLabel(subDivision, data&0xFF);
            seek(save, context);
            if(label!=null)
    ...
    private String getLabelInternal(long offset, FileContext context) throws IOException
    {
        if(offset<0 || offset*dataOffsetMultiplier>=dataLength)
            throw new IOException("Invalid label offset: 0x"+Long.toHexString(offset));
            ...
            throws IOException
    {
        ...
        switch(labelCoding)
        {
            ...
            case 9:
                ...
                break;
            default:
                System.out.println("Don't know how to decode label with coding "+labelCoding);
                return "???";
        }
        seek(save, context);
        return result;
    ...
    private String getLabel6b(long offset, FileContext context) throws IOException
    {
        seek(offset, context);
        ...
        while(true)
        {
            ...
            int c[]=
                    {b1>>2, (b1&0x3)<<4|b2>>4, (b2&0xF)<<2|b3>>6, b3&0x3F};
    ...
    private String getLabel8b(long offset, FileContext context) throws IOException
    {
        seek(offset, context);
        StringBuffer result=new StringBuffer();
        while(true)
