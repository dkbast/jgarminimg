...
public class FindObjectByPositionListener implements MapListener
{
    ...
    public FindObjectByPositionListener(int targetlong, int targetLat, int precision)
    {
    ...
    {
        if(longitude>=targetlong-precision && longitude<=targetlong+precision && latitude>=targetLat-precision
           && latitude<=targetLat+precision)
        {
            founds.add(new FoundPoint(type, subType, longitude, latitude, label, indexed));
        }
    }
        ...
        if(isNearPoints(longitudes, latitudes, nbPoints))
        {
            if(line)
            {
                if(isNearLine(longitudes, latitudes, nbPoints))
                    founds.add(new FoundPoly(type, longitudes, latitudes, nbPoints, label, true));
            }
            else
            {
                if(isInPolygon(longitudes, latitudes, nbPoints))
                {
                    founds.add(new FoundPoly(type, longitudes, latitudes, nbPoints, label, false));
                }
            }
        }
    }
    ...
    private boolean isInPolygon(int[] longitudes, int[] latitudes, int nbPoints)
    {
        ...
        return path.contains(targetlong, targetLat);
    }
    ...
    private boolean isNearLine(int[] longitudes, int[] latitudes, int nbPoints)
    {
        ...
        {
            double dist=Line2D.ptSegDist(longitudes[cpt], latitudes[cpt], longitudes[cpt+1], latitudes[cpt+1],
    ...
    private boolean isNearPoints(int[] longitudes, int[] latitudes, int nbPoints)
    {
        ...
        return CoordUtils.includedInCoordinates(targetlong, targetLat, minLon, maxLon, minLat, maxLat);
    }
