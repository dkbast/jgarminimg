 ...
 * </ul>
 * <p>Since this class is heavily used, a lot of care has been taken to avoid performance
 * problems (mainly memory allocation).
 */
public class MapTransformer<T> implements Cloneable
{
    ...
    /**
     * Use it to change the dimensions of the display area (in screen pixel).
     * That will change the amount of details show, but the same area of the map will be shown.
     * <p/>
    ...
    /**
     * @return The WGS84 coordinate (radians) of the lower left corner of the map.
     */
    ...
    /**
     * @return The WGS84 coordinate (radians) of the upper right corner of the map.
     */
     ...
     * @param longitudes the input longitudes
     * @param latitudes  the output latitudes
     * @param xPoints    the output X positions
     * @param yPoints    the output Y positions
     * @param nbPoints   the number of points to concider
     * @param tempXY     some temporary variable (avoid to do too many allocations for perf purpose)
     * @param tempCoord  some other temporary variable (avoid to do too many allocations for perf purpose)
     */
    public void garminGeo2Map(int[] longitudes, int[] latitudes, int[] xPoints, int[] yPoints, int nbPoints, Point2D.Double tempXY, T tempCoord)
    {
        for(int cpt=0; cpt<nbPoints; ++cpt)
        {
            wgs84ToMap(CoordUtils.toWGS84Rad(longitudes[cpt]), CoordUtils.toWGS84Rad(latitudes[cpt]), tempCoord, tempXY);
    ...
    {
        return minX!=Double.MAX_VALUE && minX!=maxX && minY!=maxY && width!=0 && height!=0;
    }
    ...
    public void moveMapPosition(double deltaX, double deltaY)
    {
        ...
        double scaledY=-deltaY*(maxY-minY)/(double)(height-2*margin);
        minX+=scaledX;
        maxX+=scaledX;
        minY+=scaledY;
        maxY+=scaledY;
    ...
    public void wgs84ToMapNoScale(double lon, double lat, T temp, Point2D.Double result)
    {
    ...
    {
        return (int)((maxY-d)*(height-2*margin)/(maxY-minY))+margin;
    }

    private double unscaleY(double y)
    {
        return maxY-(y-margin)*(maxY-minY)/(double)(height-2*margin);
    }

    private int scaleX(double d)
    {
        return (int)((d-minX)*(width-2*margin)/(maxX-minX))+margin;
    }

    private double unscaleX(double x)
    {
        return (x-margin)*(maxX-minX)/(double)(width-2*margin)+minX;
    }
    ...
    {
        double altAng=2.0d*Math.PI/40041d; //what angle in radian is one kilometer at the surface of the earth
    ...
    /**
     * Interface for the class converting from/to the user defined coordinate system
     * and datum to/from WGS84.
     * <p>Must be state less and thread safe.
     */
    public interface Converter<T2>
    {
         ...
         * @param result Where the result is put. Longitude in x and latitude in y.
         */
